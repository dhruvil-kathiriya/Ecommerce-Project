...
module.exports.view_category = async (req, res) => {
  try {
    ...
    let catdata = await category
      ...
      .limit(perPage)
      .skip(perPage * page);
    let totalcatData = await category
      .find({
        $or: [
          { category_name: { $regex: ".*" + search + ".*", $options: "i" } },
        ],
...
module.exports.isActive = async (req, res) => {
  try {
    if (req.params.id) {
      let active = await category.findByIdAndUpdate(req.params.id, {
        isActive: false,
      });
      if (active) {
...
module.exports.updatecategory = async (req, res) => {
  ...
  return res.render("Admin_pages/Update_category", {
